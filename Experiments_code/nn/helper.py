...
def randomized_conformal_cutoffs(series, T, alpha=0.1):
    ...
    if series.iloc[k - 1 - 1] < series.iloc[k - 1]:
        ...
        condition_3 = np.logical_and(
            tau >= cutoff_1 + cutoff_2, tau < cutoff_1 + cutoff_2 + cutoff_3
        )
...
class sample_binning_model:
    ...
    def fit(self, x_train, y_train):
        ...
        for t, x in enumerate(self.x_train_values):
            self.bin_model_lower[t] = np.quantile(
                y_train[(x_train == x).reshape(-1)], self.q_lower / 100
            )
            self.bin_model_upper[t] = np.quantile(
                y_train[(x_train == x).reshape(-1)], self.q_upper / 100
            )
    ...
    def predict(self, X_test):
        ...
        for x_test in X_test:
            ...
            if len(selected_y_train) == 0:
                raise ValueError(
                    f"No training samples found within the threshold of {self.threshold} for test point {x_test}."
                )
...
class CatBoostWrapper:
    ...
    def predict(self, X_test):
        return self.lower_model.predict(
            X_test, ntree_end=self.t + 1
        ), self.upper_model.predict(X_test, ntree_end=self.t + 1)
