...
# SampleRandomForestQuantileRegressor, KNeighborsQuantileRegressor
...
from helper import (
    generate_data,
    interval_score_loss,
    randomized_conformal_cutoffs,
    select_column_per_row,
    average_coverage,
    average_interval_width,
)
from helper import (
    sample_binning_model,
    QuantileBandwidthModel,
    QuantileRegressionNN,
    corr_coverage_widths,
    # hsic_coverage_widths,
    # wsc,
)
...
class uacqr:
    def __init__(
        ...
        uacqrs_agg="std",
        oracle_g=None,
        randomized_conformal=False,
        subsample=1,
        sample_rfqr=False,
        extraneous_quantiles=["mean"],
        uacqrs_bagging=False,
        double_bootstrapping=False,
    ):
        """
        inputs:
        ...
        if (
            ...
            and (model_params.get("dropout", 0) == 0)
            and not (model_params.get("epoch_model_tracking", False))
        ):
            ...
            print(
                "Will use dropout to generate B ensemble members. Set model_params[``epoch_model_tracking''] = True to use epoch-based heuristic instead"
            )
    ...
    def fit(self, x_train, y_train):
        ...
        if not (self.bootstrapping_for_uacqrp) and (
            self.model_type not in ["rfqr", "neural_net", "catboost"]
        ):
            raise Exception(
                "Cannot use Fast UACQR for models other than RFQR, CatBoost, and Neural Net"
            )
        ...
        for b in range(self.B):
            b_idxs = np.random.choice(
                y_train.shape[0],
                size=int(y_train.shape[0] * self.subsample),
                replace=True,
            )
            ...
            #  if self.sample_rfqr:
            #     model_b = SampleRandomForestQuantileRegressor(q=[self.q_lower/100, self.q_upper/100], random_state=self.random_state+b, n_jobs=-1, **self.uacqrp_model_params)
            # else:
            ...
            if self.model_type == "linear":
                model_b_lower = QuantileRegressor(
                    quantile=self.q_lower / 100, **self.uacqrp_model_params
                )
                model_b_upper = QuantileRegressor(
                    quantile=self.q_upper / 100, **self.uacqrp_model_params
                )
                model_b_lower.fit(x_train_b, y_train_b)
                model_b_upper.fit(x_train_b, y_train_b)
                model_b = [model_b_lower, model_b_upper]
            elif self.model_type == "lightgbm":
                model_b_lower = LGBMRegressor(
                    objective="quantile",
                    metric="quantile",
                    boosting_type="gbdt",
                    alpha=self.q_lower / 100,
                    n_jobs=-1,
                    random_state=self.random_state + b,
                    **self.uacqrp_model_params,
                )
                model_b_upper = LGBMRegressor(
                    objective="quantile",
                    metric="quantile",
                    boosting_type="gbdt",
                    alpha=self.q_upper / 100,
                    n_jobs=-1,
                    random_state=self.random_state + b,
                    **self.uacqrp_model_params,
                )
                ...
                model_b = QuantileBandwidthModel(
                    quantiles=[self.q_lower / 100, self.q_upper / 100],
                    **self.uacqrp_model_params,
                )
                model_b.fit(x_train_b, y_train_b)
            ...
            # model_b.fit(x_train_b, y_train_b)
                ...
                model_b = QuantileRegressionNN(
                    quantiles=self.modeled_quantiles,
                    random_state=self.random_state + b,
                    **self.uacqrp_model_params,
                )
                model_b.fit(x_train_b, y_train_b)
            elif self.model_type == "neural_net" and not (
                self.bootstrapping_for_uacqrp
            ):
                break
                ...
                model_b.fit(x_train_b, y_train_b)
                self.sigmahat = (
                    np.mean(np.square(model_b.predict(self, x_train) - self.y_train))
                    ** 0.5
                )
            elif self.model_type == "catboost" and self.bootstrapping_for_uacqrp:
                print(
                    "Not implemented yet to use boostrapping_for_uacqrp=True with CatBoost"
                )
        ...
        if (self.model_type == "catboost") and not (self.bootstrapping_for_uacqrp):
            catboost_model_lower = CatBoostRegressor(
                loss_function=f"Quantile:alpha={self.q_lower/100}",
                random_state=self.random_state,
                verbose=False,
                **self.model_params,
            )
            catboost_model_upper = CatBoostRegressor(
                loss_function=f"Quantile:alpha={self.q_upper/100}",
                random_state=self.random_state,
                verbose=False,
                **self.model_params,
            )
            ...
            if self.B != catboost_model_lower.tree_count_:
                print(
                    "B != number of boosting rounds. Overriding to B = number of boosting rounds"
                )
                self.B = min(
                    catboost_model_lower.tree_count_, catboost_model_upper.tree_count_
                )

            if catboost_model_lower.tree_count_ != catboost_model_upper.tree_count_:
                print(
                    "Different number of boosting rounds for upper and lower quantile regressors. Overriding B to be the minimum of the two"
                )

            self.models_B = [
                CatBoostWrapper(self.catboost_model, t) for t in range(self.B)
            ]
    ...
    def calibrate(
        self, x_calib, y_calib, inject_noise=False, cond_exp=False, noise_sd_fn=False
    ):
        n1 = x_calib.shape[0]

        calib_sorted_q_lower_ests = self.__predict_B_sorted(x_calib, lower=True)

        calib_sorted_q_lower_ests_df, scores_lower = self.__uacqrp_scores(
            y_calib, n1, calib_sorted_q_lower_ests, lower=True
        )

        calib_sorted_q_upper_ests = self.__predict_B_sorted(x_calib, lower=False)

        calib_sorted_q_upper_ests_df, scores_upper = self.__uacqrp_scores(
            y_calib, n1, calib_sorted_q_upper_ests, lower=False
        )

        if self.bootstrapping_for_uacqrp:
            cqr_scores_upper = self.transform(
                calib_sorted_q_upper_ests_df["truth"]
            ) - self.transform(calib_sorted_q_upper_ests_df[int(self.B / 2)])

            cqr_scores_lower = self.transform(
                calib_sorted_q_lower_ests_df[int(self.B / 2)]
            ) - self.transform(calib_sorted_q_upper_ests_df["truth"])

            calib_interval_widths = self.transform(
                calib_sorted_q_upper_ests_df[int(self.B / 2)]
            ) - self.transform(calib_sorted_q_lower_ests_df[int(self.B / 2)])
        else:
            cqr_scores_upper = self.transform(
                calib_sorted_q_upper_ests_df["truth"]
            ) - self.transform(self.cqr_base_model.predict(x_calib)[-1])

            cqr_scores_lower = self.transform(
                self.cqr_base_model.predict(x_calib)[0]
            ) - self.transform(calib_sorted_q_upper_ests_df["truth"])

            calib_interval_widths = self.transform(
                self.cqr_base_model.predict(x_calib)[-1]
            ) - self.transform(self.cqr_base_model.predict(x_calib)[0])

        uacqrs_spread_upper = calib_sorted_q_upper_ests_df.iloc[:, :-2].agg(
            self.uacqrs_agg, axis=1
        )
        uacqrs_spread_lower = calib_sorted_q_lower_ests_df.iloc[:, :-2].agg(
            self.uacqrs_agg, axis=1
        )
        ...
        if self.oracle_g:
            ...
            true_quantile_lower = cond_exp(x_calib) - norm.ppf(
                self.q_upper / 100, 0, noise_sd_fn(x_calib)
            )
            true_quantile_upper = cond_exp(x_calib) + norm.ppf(
                self.q_upper / 100, 0, noise_sd_fn(x_calib)
            )
        ...
        if inject_noise:
            noise_upper_variance = (
                calib_sorted_q_upper_ests_df.iloc[:, :-2]
                .agg(self.uacqrs_agg, axis=1)
                .var()
                * inject_noise
            )
            noise_lower_variance = (
                calib_sorted_q_lower_ests_df.iloc[:, :-2]
                .agg(self.uacqrs_agg, axis=1)
                .var()
                * inject_noise
            )
            ...
            noise_upper = np.random.normal(
                size=calib_sorted_q_upper_ests_df.shape[0],
                scale=noise_upper_variance**0.5,
            )
            noise_lower = np.random.normal(
                size=calib_sorted_q_lower_ests_df.shape[0],
                scale=noise_lower_variance**0.5,
            )
        ...
        if self.uacqrs_bagging:
            uacqrs_scores_upper = self.transform(
                calib_sorted_q_upper_ests_df["truth"]
            ) - self.transform(calib_sorted_q_upper_ests_df[int(self.B / 2)])
            uacqrs_scores_lower = self.transform(
                calib_sorted_q_lower_ests_df[int(self.B / 2)]
            ) - self.transform(calib_sorted_q_upper_ests_df["truth"])
        ...
        cqr_scores = pd.DataFrame(
            {"upper": cqr_scores_upper, "lower": cqr_scores_lower}
        )

        uacqrs_scores = pd.DataFrame(
            {"upper": uacqrs_scores_upper, "lower": uacqrs_scores_lower}
        )

        cqrr_scores = pd.DataFrame(
            {"upper": cqrr_scores_upper, "lower": cqrr_scores_lower}
        )
        ...
        self.score_threshold = (
            ...
            .iloc[math.ceil((1 - self.alpha) * (n1 + 1)) - 1]
        )
        self.cqr_score_threshold = (
            cqr_scores["combined"]
            .sort_values(ascending=True)
            .iloc[math.ceil((1 - self.alpha) * (n1 + 1)) - 1]
        )
        self.uacqrs_score_threshold = (
            uacqrs_scores["combined"]
            .sort_values(ascending=True)
            .iloc[math.ceil((1 - self.alpha) * (n1 + 1)) - 1]
        )
        self.cqrr_score_threshold = (
            cqrr_scores["combined"]
            .sort_values(ascending=True)
            .iloc[math.ceil((1 - self.alpha) * (n1 + 1)) - 1]
        )
        ...
        if self.randomized_conformal:
            print(
                "Warning: score threshold attributes at this point are not randomized. They will be after making predictions"
            )
    ...
    def evaluate(self, x_test, y_test, oqr_metrics=False):
        ...
        bounds_df = self.predict_uacqr(x_test)
        methods = ["UACQR-P", "UACQR-S", "CQR", "CQR-r", "Base"]
        metrics = dict()

        metrics["interval_score_loss"] = {
            method: interval_score_loss(
                bounds_df[method]["upper"].values,
                bounds_df[method]["lower"].values,
                y_test,
                alpha=self.alpha,
            ).mean()
        ...
        metrics["average_length_test"] = {
            method: average_interval_width(
                bounds_df[method]["upper"].values, bounds_df[method]["lower"].values
            )
            for method in methods
        }
        ...
        for outer_key, inner_dict in metrics.items():
            for inner_key, inner_value in inner_dict.items():
                setattr(
                    self,
                    f"{inner_key.replace('-','').lower()}_{outer_key}",
                    inner_value,
                )

        return metrics

    def calculate_conditional_coverage(
        self,
        cond_exp=None,
        noise_sd_fn=None,
        plot=True,
        ...
        column_by_feature_importance=False,
        ecdf=False,
        metric="conditional_coverage",
    ):

        if (
            not (hasattr(self, "test_cover_rates"))
            and cond_exp
            and metric == "conditional_coverage"
        ):
            miscover_below_rate = norm.cdf(
                self.test_y_lower,
                loc=cond_exp(self.x_test),
                scale=noise_sd_fn(self.x_test),
            )
            miscover_below_rate_cqr = norm.cdf(
                self.test_y_lower_cqr,
                loc=cond_exp(self.x_test),
                scale=noise_sd_fn(self.x_test),
            )
            miscover_below_rate_uacqrs = norm.cdf(
                self.test_y_lower_uacqrs,
                loc=cond_exp(self.x_test),
                scale=noise_sd_fn(self.x_test),
            )
            miscover_below_rate_cqrr = norm.cdf(
                self.test_y_lower_cqrr,
                loc=cond_exp(self.x_test),
                scale=noise_sd_fn(self.x_test),
            )
            if not (self.bootstrapping_for_uacqrp):
                miscover_below_rate_base = norm.cdf(
                    self.test_y_lower_base,
                    loc=cond_exp(self.x_test),
                    scale=noise_sd_fn(self.x_test),
                )
            else:
                miscover_below_rate_base = norm.cdf(
                    self.test_y_lower_median,
                    loc=cond_exp(self.x_test),
                    scale=noise_sd_fn(self.x_test),
                )

            miscover_above_rate = 1 - norm.cdf(
                self.test_y_upper,
                loc=cond_exp(self.x_test),
                scale=noise_sd_fn(self.x_test),
            )
            miscover_above_rate_cqr = 1 - norm.cdf(
                self.test_y_upper_cqr,
                loc=cond_exp(self.x_test),
                scale=noise_sd_fn(self.x_test),
            )
            miscover_above_rate_uacqrs = 1 - norm.cdf(
                self.test_y_upper_uacqrs,
                loc=cond_exp(self.x_test),
                scale=noise_sd_fn(self.x_test),
            )
            miscover_above_rate_cqrr = 1 - norm.cdf(
                self.test_y_upper_cqrr,
                loc=cond_exp(self.x_test),
                scale=noise_sd_fn(self.x_test),
            )
            if not (self.bootstrapping_for_uacqrp):
                miscover_above_rate_base = 1 - norm.cdf(
                    self.test_y_upper_base,
                    loc=cond_exp(self.x_test),
                    scale=noise_sd_fn(self.x_test),
                )
            else:
                miscover_above_rate_base = 1 - norm.cdf(
                    self.test_y_upper_median,
                    loc=cond_exp(self.x_test),
                    scale=noise_sd_fn(self.x_test),
                )

            self.test_cover_rates = 1 - miscover_below_rate - miscover_above_rate
            self.test_cover_rates_cqr = (
                1 - miscover_below_rate_cqr - miscover_above_rate_cqr
            )
            self.test_cover_rates_uacqrs = (
                1 - miscover_below_rate_uacqrs - miscover_above_rate_uacqrs
            )
            self.test_cover_rates_cqrr = (
                1 - miscover_below_rate_cqrr - miscover_above_rate_cqrr
            )
            self.test_cover_rates_base = (
                1 - miscover_below_rate_base - miscover_above_rate_base
            )
        else:
            if metric == "conditional_coverage":
                self.test_cover_rates = (self.test_y_lower <= self.y_test) & (
                    self.test_y_upper >= self.y_test
                )
                self.test_cover_rates_cqr = (self.test_y_lower_cqr <= self.y_test) & (
                    self.test_y_upper_cqr >= self.y_test
                )
                self.test_cover_rates_uacqrs = (
                    self.test_y_lower_uacqrs <= self.y_test
                ) & (self.test_y_upper_uacqrs >= self.y_test)
                self.test_cover_rates_cqrr = (self.test_y_lower_cqrr <= self.y_test) & (
                    self.test_y_upper_cqrr >= self.y_test
                )
                self.test_cover_rates_base = (self.test_y_lower_base <= self.y_test) & (
                    self.test_y_upper_base >= self.y_test
                )
            elif metric == "average_length_test":
                self.test_cover_rates = self.test_y_upper - self.test_y_lower
                self.test_cover_rates_cqr = (
                    self.test_y_upper_cqr - self.test_y_lower_cqr
                )
                self.test_cover_rates_uacqrs = (
                    self.test_y_upper_uacqrs - self.test_y_lower_uacqrs
                )
                self.test_cover_rates_cqrr = (
                    self.test_y_upper_cqrr - self.test_y_lower_cqrr
                )
                self.test_cover_rates_base = (
                    self.test_y_upper_base - self.test_y_lower_base
                )
        ...
        if cond_exp:
            ...
            if column_by_feature_importance:
                sorting_column = np.argmax(
                    self.self.cqr_base_model.feature_importances_
                )
            if ecdf:
                x_df[sorting_column] = (
                    x_df[sorting_column].rank(pct=True).round(round_to)
                )
            else:
                x_df[sorting_column] = (
                    x_df[sorting_column].rank(pct=True).round(round_to)
        ...
        self.conditional_coverage = x_df.groupby(sorting_column)[
            included_methods
        ].mean()

        if plot:
            self.conditional_coverage.plot(
                title=title,
    ...
    def plot(self, cond_exp, noise_sd_fn, feature_for_x_axis=0):
        colors = {
            ...
            "CQR": "tab:green",
            "UACQR-S": "tab:red",
            "Base Estimator": "tab:purple",
            "CQR-m with oracle info": "tab:brown",
            "UACQR-S with oracle info": "tab:pink",
            "Truth": "tab:blue",
            "Median Estimator": "tab:purple",
            "RFQR": "tab:purple",
            "CQR-r": "tab:brown",
        }
        ...
        ax.plot(
            x_column,
            (
                cond_exp(self.x_test)
                - norm.ppf(self.q_upper / 100, 0, noise_sd_fn(self.x_test))
            )[idxs],
            label="Truth",
            linewidth=3,
            color=colors["Truth"],
        ...
        ax.plot(
            x_column,
            self.test_y_lower[idxs],
            label="UACQR-P",
            alpha=1,
            color=colors["UACQR-P"],
        )
        ax.plot(
            x_column,
            self.test_y_lower_cqr[idxs],
            label="CQR",
            alpha=1,
            color=colors["CQR"],
        )
        ax.plot(
            x_column,
            self.test_y_lower_uacqrs[idxs],
            label="UACQR-S",
            alpha=1,
            color=colors["UACQR-S"],
        )
        ax.plot(
            x_column,
            self.test_y_lower_cqrr[idxs],
            label="CQR-r",
            alpha=1,
            color=colors["CQR-r"],
        )
        if self.bootstrapping_for_uacqrp:
            ax.plot(
                x_column,
                self.test_y_lower_median[idxs],
                label="Median Estimator",
                alpha=1,
                color=colors["Median Estimator"],
            )
        else:
            ax.plot(
                x_column,
                self.test_y_lower_base[idxs],
                label="Base Estimator",
                alpha=1,
                color=colors["Base Estimator"],
            )
    ...
    def plot_simple(
        ...
        if expanded:
            fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(
                2, 2, sharey=sharey, figsize=(15, 12)
            )
    ...
    def __predict_individual_model(self, x_calib, model, lower=True):
        ...
        if self.model_type == "rfqr":
            ...
            self.model_type == "sample_binning"
            or self.model_type == "bandwidth"
            or self.model_type == "knn"
        ):
            return model.predict(x_calib)[i]
        elif (self.model_type == "neural_net") and self.bootstrapping_for_uacqrp:
            return model.predict(x_calib)[i]
    ...
    def __predict_B_sorted(self, x_test, lower=True):
        ...
        for t, model in enumerate(self.models_B):
            q_lower_uacqrp[:, t] = self.__predict_individual_model(
                x_test, model, lower=lower
            )

        if self.model_type == "neural_net" and not (self.bootstrapping_for_uacqrp):
            q_lower_uacqrp = self.nn_model.predict(x_test, ensembling=self.B + 1)[
                0 if lower else -1
            ]
    ...
    def __uacqrp_scores(self, y_calib, n1, calib_sorted_ests, lower=True):
        calib_sorted_q_ests_df = pd.DataFrame(
            data=calib_sorted_ests, index=range(n1), columns=range(self.B + 1)
        )
